...
const seeDetails = (data) => {
  // Ectract modal element
  const elm = document.getElementById("modal-dialog")
  const div = document.createElement("div");
  div.classList.add('modal-content');

  // Create element data to show on the modal
  div.innerHTML = `
...
// Add to cart cotrol flow
let count = 0;

const addToCart = (id, price) => {
  count = count + 1;
  updatePrice("price", price); // Update price after select a product

  // Apply tax and delivery calculation
  updateTaxAndCharge();
...
// Extract value from a spcific element
